...
void print_results(float array[N], int tid, int section) 
{
  ...
  //FIX: Have removed the #pramga omp barrier because as only one thread can enter a critical section and the barrier command will wait for all the threads, the program will hang as the rest of the threads will never reach the barrier and the program will not move forward
