...
void MMult0_p(long m, long n, long k, double *a, double *b, double *c) {
  long size_ = m*n;
...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  for(long i_ = 0; i_ < m; i_+= BLOCK_SIZE)
  {
    for(long j_ = 0; j_ < n; j_+= BLOCK_SIZE)
    {
      for(long k_ = 0; k_ < k; k_+= BLOCK_SIZE)
      {
        for (long i = i_; i < i_+ BLOCK_SIZE; i++) {
          for (long j = j_; j < j_+ BLOCK_SIZE; j++) {
            for (long p = k_; p < k_+ BLOCK_SIZE; p++) {  
              double A_ip = a[i+p*m];
              double B_pj = b[p+j*k];
              double C_ij = c[i+j*m];
              C_ij = C_ij + A_ip * B_pj;
...
void MMult1_p(long m, long n, long k, double *a, double *b, double *c) {
 long size_ = m*n;
  #pragma omp parallel num_threads(nrT)
  {
