...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = (2 * m * n * k * NREPEATS); // TODO: calculate from m, n, k, NREPEATS, time
    double floprate = flops / time / 1e9;
    double bandwidth = ((2 * (m * n)) * (k + 1) * NREPEATS * sizeof(double)) / time / 1e9; // TODO: calculate from m, n, k, NREPEATS, time
